...
use consensus::{ProofOfTime, Validator, HalvingSchedule, VestingSchedule};
use tokens::{TokenRegistry, PNP16Token, AssetClass};
use escrow::MVault;
...
async fn main() {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let seed = format!("{}_{}", latest.hash, Utc::now().timestamp_nanos_opt().unwrap_or(0));
            ...
            match bc_clone.commit_block(block) {
                Ok(_)  => info!("Block #{height} committed | {bh}... | {tc} txs | validator: {validator_addr}"),
        ...
        loop {
            ...
            for reward in &rewards {
                ...
                let tx = Transaction::new(
                    ...
                    "ValidatorRewardSystem", &reward.validator_address,
                    reward.amount,
...
fn print_startup_summary(state: &AppState) {
    ...
    info!("Chain Height: {} | Validators: {} | Tokens: {} | Wallets: {}",
        stats.block_height,
...
fn seed_demo_data(state: &AppState) {
    ...
    {
        let pecu_gold = PNP16Token::new("PecuGold","PGLD",18,
            1_000_000_000_000_000_000_000_000u128,
            Some(10_000_000_000_000_000_000_000_000u128),
            AssetClass::PhysicalCommodity, &alice_addr, "DAK_DEMO_001");
        let pecu_realty = PNP16Token::new("PecuRealty","PRTY",6,
            100_000_000_000u128, None,
            AssetClass::FractionalRealEstate, &bob_addr, "DAK_DEMO_002");
...

    {
        ...
        let tx = Transaction::new(TransactionType::Transfer,
            &alice_addr, &bob_addr, 1_000_000_000_000_000_000u128,
            Some("Demo transfer Alice->Bob".to_string()),
            None, false, None, None, nonce_a);
        let _ = state.blockchain.add_to_mempool(tx);

        let nonce_b = state.blockchain.get_nonce(&bob_addr);
        let escrow_tx = Transaction::new(TransactionType::Escrow,
            &bob_addr, &carol_addr, 500_000_000_000_000_000u128,
            Some("Real estate deposit".to_string()),
            Some("Property deposit escrow".to_string()),
            true, Some(Utc::now().timestamp() + 7 * 86400),
            None, nonce_b);
