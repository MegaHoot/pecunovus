...
impl ChainStorage {
    pub fn open(path: &str) -> Result<Self, sled::Error> {
        ...
        Ok(ChainStorage { db, blocks_tree, txs_tree, tokens_tree, wallets_tree, state_tree })
    }
    ...
    pub fn save_block(&self, block: &Block) -> Result<(), sled::Error> {
        ...
        self.blocks_tree.insert(block.hash.as_bytes(), Self::encode(block))?;
        for tx in &block.transactions {
            self.txs_tree.insert(tx.tx_hash.as_bytes(), Self::encode(tx))?;
        }
        Ok(())
    }
    ...
    pub fn get_block_by_height(&self, height: u64) -> Option<Block> {
        ...
        self.blocks_tree.get(key).ok().flatten().and_then(|b| Self::decode(&b))
    }

    pub fn get_block_by_hash(&self, hash: &str) -> Option<Block> {
        self.blocks_tree.get(hash.as_bytes()).ok().flatten().and_then(|b| Self::decode(&b))
    }
    ...
    pub fn get_transaction(&self, tx_hash: &str) -> Option<Transaction> {
        self.txs_tree.get(tx_hash.as_bytes()).ok().flatten().and_then(|b| Self::decode(&b))
    }

    pub fn save_token(&self, token: &PNP16Token) -> Result<(), sled::Error> {
        self.tokens_tree.insert(token.contract_address.as_bytes(), Self::encode(token))?;
        Ok(())
    }

    pub fn get_token(&self, contract_address: &str) -> Option<PNP16Token> {
        self.tokens_tree.get(contract_address.as_bytes()).ok().flatten().and_then(|b| Self::decode(&b))
    }

    pub fn save_wallet(&self, wallet: &Wallet) -> Result<(), sled::Error> {
        self.wallets_tree.insert(wallet.keypair.evm_address.as_bytes(), Self::encode(wallet))?;
        Ok(())
    }

    pub fn get_wallet(&self, address: &str) -> Option<Wallet> {
        self.wallets_tree.get(address.as_bytes()).ok().flatten().and_then(|b| Self::decode(&b))
    }
